...
task("deploy:MultiSigRecovery", "Deploys the MultiSig")
  ...
  .setAction(async ({ pactusFoundationAddress, wraptoTeamAddress }, { ethers }: HardhatRuntimeEnvironment) => {
    ...
    const MultiSigFactory = await ethers.getContractFactory("MultiSigRecovery");
